...
function SettingsMenu() {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className={`flex min-h-11 items-center justify-center rounded-full border px-3 py-2 transition hover:-translate-y-0.5 active:scale-[0.98] ${
          settingsActive
            ...
            : "border-stone-300 bg-card text-stone-700 hover:border-stone-400 hover:text-stone-900 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-200 dark:hover:bg-stone-700"
        }`}
      >
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-44 rounded-2xl border border-[var(--border)] bg-card py-2 shadow-lg">
          <Link
            href="/app/settings"
...
export function AppShell({ children, unreadNotes = 0 }: { children: React.ReactNode; unreadNotes?: number }) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 pb-24 pt-6 sm:px-6 sm:pb-10 sm:pt-8">
      <header className="mb-6 rounded-3xl border border-[var(--border)]/80 bg-card/90 p-4 shadow-sm backdrop-blur sm:mb-8 sm:px-6 sm:py-4">
            ...
            return (
              <Link
                ...
                className={`relative rounded-full border px-3 py-1.5 font-medium transition ${
                  active
                    ? "btn-accent shadow-sm"
                    : "border-stone-300 bg-card text-stone-700 hover:-translate-y-0.5 hover:border-stone-400 hover:bg-stone-50 hover:text-stone-900 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-200 dark:hover:bg-stone-700"
                }`}
              >
                {item.label}
                {showBadge && (
      ...
      <nav
        ...
        className="fixed inset-x-0 bottom-0 z-30 border-t border-[var(--border)] bg-card/95 px-3 py-2 backdrop-blur sm:hidden"
      >
